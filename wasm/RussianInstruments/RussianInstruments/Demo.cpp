...
// #define DEMO_SIZEX	1000
// #define DEMO_SIZEY	600
#define DEMO_SIZEX 1024
#define DEMO_SIZEY 1024
...
int loadDemoData(NVGcontext *vg) {
    ...
    for (i = 0; i < 12; i++) {
        ...
        gData.images[i] = nvgCreateImage(vg, file, 0);
        if (gData.images[i] == 0) {
    ...
    gData.fontIcons = nvgCreateFont(vg, "icons", "./data/entypo.ttf");
    if (gData.fontIcons == -1) {
        fprintf(stderr, "Could not add font icons.\n");
        return -1;
    }
    gData.fontNormal = nvgCreateFont(vg, "sans", "./data/Roboto-Regular.ttf");
    if (gData.fontNormal == -1) {
        fprintf(stderr, "Could not add font italic.\n");
        return -1;
    }
    gData.fontBold = nvgCreateFont(vg, "sans-bold", "./data/Roboto-Bold.ttf");
    if (gData.fontBold == -1) {
        fprintf(stderr, "Could not add font bold.\n");
        return -1;
    }
    gData.fontEmoji = nvgCreateFont(vg, "emoji", "./data/NotoEmoji-Regular.ttf");
    if (gData.fontEmoji == -1) {
        fprintf(stderr, "Could not add font emoji.\n");
        return -1;
    }
    nvgAddFallbackFontId(vg, gData.fontNormal, gData.fontEmoji);
    nvgAddFallbackFontId(vg, gData.fontBold, gData.fontEmoji);

    return 0;
};

void draw(NVGcontext *ctx,
          double mx, double my, double t, double dt,
    ...
    nvgBeginFrame(ctx, winWidth, winHeight, pxRatio);
    {
        ...
        nvgFillColor(ctx, nvgRGBAf(0.3f, 0.3f, 0.32f, 1.0f));
        nvgBeginPath(ctx);
        nvgRect(ctx, 0, 0, winWidth, winHeight);
        nvgFill(ctx);
        ...
        drawColorwheel(ctx, width - 300, height - 300, 250.0f, 250.0f, t);
        ...
        nvgSave(ctx);
        if (blowup) {
            nvgRotate(ctx, sinf(t * 0.3f) * 5.0f / 180.0f * NVG_PI);
        ...
        drawButton(ctx, ICON_LOGIN, "Sign in", x + 138, y, 140, 28, nvgRGBA(0, 96, 128, 255));
        y += 45;
        ...
        drawSlider(ctx, 0.4f, x, y, 170, 28);
        y += 55;

        drawButton(ctx, ICON_TRASH, "Delete", x, y, 160, 28, nvgRGBA(128, 16, 8, 255));
        drawButton(ctx, 0, "Cancel", x + 170, y, 110, 28, nvgRGBA(0, 0, 0, 0));

        // Thumbnails box
        drawThumbnails(ctx, 365, popy - 30, 160, 300, gData.images, 12, t);
...
MSFS_CALLBACK bool Demo_gauge_callback(FsContext ctx, int service_id, void *pData) {
    ...
    case PANEL_SERVICE_POST_INSTALL: {
        ...
        g_DemoVars.m_nvgctx = nvgctx;
        if (loadDemoData(nvgctx) == -1) {
            fprintf(stderr, "Failed to load demo data\n");
            return false;
        }
    } break;
    case PANEL_SERVICE_PRE_DRAW: {
        sGaugeDrawData *p_draw_data = (sGaugeDrawData *)pData;
        NVGcontext *nvgctx = g_DemoVars.m_nvgctx;
        draw(nvgctx, p_draw_data->mx, p_draw_data->my, p_draw_data->t, p_draw_data->dt, p_draw_data->winWidth, p_draw_data->winHeight, p_draw_data->fbWidth, p_draw_data->fbHeight);
        return true;
    } break;
    case PANEL_SERVICE_PRE_KILL: {
        ...
        if (nvgctx == nullptr)
            return false;
        for (int i = 0; i < 12; i++)
            nvgDeleteImage(nvgctx, gData.images[i]);
